...
async function salvarAuditoria(outputPrefix, results, totalPlanejado) {
  ...
  await writeFile(`${output}.json`, `${JSON.stringify({ objetivo: `Inativar unidades das páginas 1 a ${LAST_PAGE}`, total_planejado: totalPlanejado, total_processado: results.length, resumo: summary, resultados: results }, null, 2)}\n`, 'utf8')
  await writeFile(`${output}.csv`, csv, 'utf8')
}

export async function inativarUnidadesPaginas1a10(outputPrefix = 'output/inativacao_unidades_paginas_1_a_10') {
  const report = JSON.parse(await readFile(REPORT_PATH, 'utf8'))
  const targets = report.unidades.filter((unit) => unit.pagina_sgc <= LAST_PAGE)
  if (!targets.length) throw new Error(`Nenhuma unidade das páginas 1 a ${LAST_PAGE} foi encontrada no relatório.`)

  console.log(`Lote selecionado: ${targets.length} unidades das páginas 1 a ${LAST_PAGE}.`)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  await inativarUnidadesPaginas1a10()
}

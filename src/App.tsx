...
import { shortenCourseForWhatsApp } from './data/courseWhatsAppAliases'
...
function ActionIcon({name}:{name:'copy'|'import'}){
...
function CourseChart({rows,summaryData,scope,asOf,period,onPeriodChange}:{rows:CourseRow[];summaryData:Enrollment[];scope:string;asOf:string;period:CoursePeriod;onPeriodChange:(period:CoursePeriod)=>void}){
 ...
 const shareSummary=()=>{
  ...
  const sections=periodSections.map(([value,label])=>{
   ...
   return `${label}\n${grouped.map(item=>{const demand=item.vacancies?(item.paid+item.unpaid)/item.vacancies:0;return `*${shortenCourseForWhatsApp(chartCourseName(item.course))} | ${demand.toFixed(1).replace('.',',')}x*\n↳ ${number.format(item.paid)} pagos | ${number.format(item.unpaid)} não pagos`}).join('\n\n')}`
  }).filter(Boolean)
  const periodDivider='──────────'
  const separatedSections=sections.flatMap((section,index)=>index?[periodDivider,'',section]:[section])
  return [`*VESTIBULINHO 2027*`,`${whatsappIcons.location} ${scope.replace('Sao','São')} · ${generatedAt}`,'',...separatedSections,'',`${whatsappIcons.target} Meta: 1,5 candidatos por vaga`].join('\n')
 }
...
<div className="course-summary-actions"><button className="icon-action" type="button" onClick={copySummary} aria-label="Copiar resumo" title="Copiar resumo"><ActionIcon name="copy"/></button><span className="sr-only" aria-live="polite">{summaryFeedback}</span></div>
...
export default function App(){
 ...
 const demandBands=presentialOfferData.filter(item=>item.vacancies>0).reduce((r,item)=>{r[offerStatusFor(item)]++;return r},{comfortable:0,attention:0,low:0})
...
<div className="enrollment-progress" role="progressbar" aria-label={`Inscrições: ${number.format(totals.paid)} pagos e ${number.format(totals.unpaid)} não pagos; ${number.format(totals.trainee)} treineiros já incluídos no total`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={total?Math.round(totals.paid/total*100):0}>
...
{analysisScope.trainees&&<span className="enrollment-trainee-detail">Inclui {number.format(totals.trainee)} treineiros no total</span>}
</div>
</div>
</div>
</article>
<article className="kpi-card demand-kpi">
<div className="demand-heading"><p>Vagas e demanda</p><span title={enrollmentScope}>Recorte selecionado</span></div>
<div className="demand-copy"><div className="demand-content"><div className="demand-values"><strong>{number.format(totals.vacancies)}</strong><small>vagas no recorte</small></div><small>{number.format(total)} inscritos no recorte</small></div></div>
<div className="demand-badge" aria-label={demandRatio===null?'Demanda não aplicável: não há vagas no recorte':`${demandRatio.toFixed(2).replace('.',',')} inscritos por vaga`}>
...
<div><strong>{demandRatio===null?'N/A':`${demandRatio.toFixed(2).replace('.',',')}x`}</strong><span>{demandRatio===null?'sem vagas no recorte':'inscritos/vaga'}</span></div>
</div>
</article>
...
{analysisTab==='evolution'?<TrendChart data={data} target={totals.target}/>:analysisTab==='courses'?<CourseChart rows={courseRows} summaryData={data.filter(item=>!item.isTrainee)} scope={whatsappScope} asOf={rangeEndAt} period={coursePeriod} onPeriodChange={setCoursePeriod}/>:<div className="regional-table analysis-regional-table">
...
<footer className="dashboard-footer">Correções e atualizações: rafael.ruiz@cps.sp.gov.br · CGETEC — ASCA
</footer>
</div>
}

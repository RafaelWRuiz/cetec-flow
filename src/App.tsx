...
<option key={item} value={item}>{label==='Etec'?(etecs.find(etec=>etec.name===item)?.label??item):item}</option>)}</select>
</label>
}
...
export default function App(){
 const [filters,setFilters]=useState<Filters>(initialFilters); const [selectedEtec,setSelectedEtec]=useState(''); const [geographicScope,setGeographicScope]=useState<{label:string;etecs:string[]}|null>(null); const [mapResetKey,setMapResetKey]=useState(0); const [analysisTab,setAnalysisTab]=useState<'evolution'|'regional'>('evolution'); const [statusFilter,setStatusFilter]=useState<OfferStatus|null>(null)
...
<button type="button" onClick={clearFilters}>Atualizar</button>
</div>
</header>
...
<SelectFilter label="Etec" value={filters.etec} values={options.etec} onChange={value=>updateFilter('etec',value)}/>
...
<div><p>Inscrições: <b>{number.format(total)} total</b></p><span>{number.format(totals.paid)} pagos · {total?`${Math.round(totals.paid/total*100)}%`:'0%'}</span><small>{number.format(totals.trainee)} treineiros</small></div>
<div className="mini-donut" style={{'--paid':`${total?totals.paid/total*360:0}deg`} as React.CSSProperties}><b>{total?`${Math.round(totals.paid/total*100)}%`:'0%'}</b><small>pagos</small></div>
</article>
<article className="kpi-card demand-kpi">
<p>Vagas e demanda</p>
<div className="demand-values"><strong>{number.format(totals.vacancies)} <small>vagas</small></strong><span><b>{totals.vacancies?`${(totals.regular/totals.vacancies).toFixed(1)}x`:'0x'}</b> demanda efetiva</span></div>
<div className="demand-scale" style={{'--demand':`${Math.min(100,Math.max(0,(totals.vacancies?totals.regular/totals.vacancies:0)/2*100))}%`} as React.CSSProperties}><i/><b>1,0x</b></div>
<small>{number.format(totals.regular)} pagos</small>
</article>
...
<div className="status-list">{offerStatus.map(([label,value,tone],index)=>
<button type="button" key={label} className={`${tone}${statusFilter===(['comfortable','attention','low'] as OfferStatus[])[index]?' active':''}`} onClick={()=>{const next=(['comfortable','attention','low'] as OfferStatus[])[index];setStatusFilter(current=>current===next?null:next)}}>
<i className={`status ${tone}`}/>
<span>{label}</span>
<b>{number.format(value)} · {regularOfferCount?`${(value/regularOfferCount*100).toFixed(1).replace('.',',')}%`:'0,0%'}</b>
</button>)}</div>
</article>
</section>
...
<footer>Fonte: snapshot local 2026.2 <span>Centro Paula Souza</span>
</footer>
</div>
}

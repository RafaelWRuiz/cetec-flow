...
export default function App(){
 const [filters,setFilters]=useState<Filters>(initialFilters); const [selectedEtec,setSelectedEtec]=useState(''); const [geographicScope,setGeographicScope]=useState<{label:string;etecs:string[]}|null>(null); const [mapResetKey,setMapResetKey]=useState(0); const [analysisTab,setAnalysisTab]=useState<'evolution'|'regional'>('evolution'); const [statusFilter,setStatusFilter]=useState<OfferStatus|null>(null); const [dashboard,setDashboard]=useState<DashboardPayload|null>(null); const [importOpen,setImportOpen]=useState(false); const [sessionEmail,setSessionEmail]=useState(''); const [importEmail,setImportEmail]=useState(''); const [importFile,setImportFile]=useState<File|null>(null); const [importMessage,setImportMessage]=useState(''); const [importing,setImporting]=useState(false)
 useEffect(()=>{let cancelled=false;void fetch('/api/dashboard-data').then(async response=>response.ok?response.json():null).then(payload=>{if(cancelled||!payload?.snapshots?.length)return;const snapshots=payload.snapshots as SnapshotSeries[];setDashboard({enrollments:snapshots.at(-1)!.enrollments,etecs:payload.etecs as EtecPoint[],snapshotSeries:snapshots,sourceMetadata:payload.sourceMetadata as typeof staticSourceMetadata})}).catch(()=>{});return()=>{cancelled=true}},[])
 useEffect(()=>{if(!supabase)return undefined;void supabase.auth.getSession().then(({data})=>setSessionEmail(data.session?.user.email??''));const {data:{subscription}}=supabase.auth.onAuthStateChange((_event,next)=>setSessionEmail(next?.user.email??''));return()=>subscription.unsubscribe()},[])
 ...
 const requestImportAccess=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();if(!supabase){setImportMessage('Configure as chaves públicas do Supabase para habilitar o acesso.');return}setImporting(true);setImportMessage('');const {error}=await supabase.auth.signInWithOtp({email:importEmail.trim(),options:{emailRedirectTo:window.location.origin}});setImporting(false);setImportMessage(error?'Não foi possível enviar o link de acesso.':'Enviamos um link de acesso para seu e-mail. Abra-o e volte a esta página.')}
 const uploadImport=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();if(!importFile||!supabase){setImportMessage('Selecione a planilha para continuar.');return}const {data:{session}}=await supabase.auth.getSession();if(!session){setImportMessage('Seu acesso expirou. Solicite um novo link.');return}setImporting(true);setImportMessage('Validando e importando a planilha...');try{const response=await fetch('/api/import-inscricoes',{method:'POST',headers:{Authorization:`Bearer ${session.access_token}`,'Content-Type':importFile.type||'application/vnd.ms-excel','X-File-Name':encodeURIComponent(importFile.name)},body:importFile});const payload=await response.json() as {error?:string;records?:number};if(!response.ok)throw new Error(payload.error??'Não foi possível importar a planilha.');setImportMessage(`Importação publicada com ${number.format(payload.records??0)} ofertas. Atualizando o painel...`);window.setTimeout(()=>window.location.reload(),900)}catch(error){setImportMessage(error instanceof Error?error.message:'Não foi possível importar a planilha.')}finally{setImporting(false)}}
...
{importOpen&&<div className="import-backdrop" role="presentation"><section className="import-dialog" role="dialog" aria-modal="true" aria-labelledby="import-title"><button className="import-close" type="button" aria-label="Fechar" onClick={()=>setImportOpen(false)}>×</button><p className="import-eyebrow">Atualização de dados</p><h2 id="import-title">Importar planilha</h2>{sessionEmail?<><p className="import-copy">A versão anterior continuará disponível até esta planilha ser validada e publicada.</p><p className="import-account">Conectado como <strong>{sessionEmail}</strong></p><form onSubmit={uploadImport}><label className="import-file">Arquivo .xls<input type="file" accept=".xls,application/vnd.ms-excel" onChange={event=>setImportFile(event.target.files?.[0]??null)}/><span>{importFile?.name??'Selecionar exportação do Vestibulinho'}</span></label><button className="import-submit" type="submit" disabled={importing}>{importing?'Importando...':'Validar e publicar'}</button></form><button className="import-signout" type="button" onClick={()=>void supabase?.auth.signOut()}>Sair desta conta</button></>:<><p className="import-copy">Entre com seu e-mail institucional para liberar a importação. Apenas e-mails autorizados conseguem publicar dados.</p><form onSubmit={requestImportAccess}><label className="import-email">E-mail institucional<input type="email" value={importEmail} onChange={event=>setImportEmail(event.target.value)} placeholder="nome@cps.sp.gov.br" required/></label><button className="import-submit" type="submit" disabled={importing}>{importing?'Enviando...':'Enviar link de acesso'}</button></form></>}<p className="import-message" role="status">{importMessage}</p></section></div>}

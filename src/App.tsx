...
<div className="course-plot-shell"><div className="course-y-axis" aria-hidden="true"><span className="course-y-max">{scaleMax.toFixed(1).replace('.',',')}x</span><span className="course-y-target" style={{bottom:`${targetHeight}%`}}>1,5x</span><span className="course-y-zero">0</span></div><div className="course-plot" ref={plotRef} role="img" aria-label="Demanda de inscritos pagos, não pagos e meta por curso">
{rows.map(row=>{const totalDemand=row.vacancies?(row.paid+row.unpaid)/row.vacancies:0;const paidHeight=Math.min(row.vacancies?row.paid/row.vacancies/scaleMax*100:0,100);const unpaidHeight=Math.min(row.vacancies?row.unpaid/row.vacancies/scaleMax*100:0,100);return <div className="course-bar-group" data-course-demand={totalDemand} key={row.course}><div className="course-bar-values"><b>{number.format(row.paid+row.unpaid)}</b><span>{row.vacancies?`${totalDemand.toFixed(1)}x`:'N/A'}</span></div><div className="course-bar-track"><i className="course-bar-paid" style={{height:`${paidHeight}%`}}/><i className="course-bar-unpaid" style={{bottom:`${paidHeight}%`,height:`${unpaidHeight}%`}}/><i className="course-bar-target" style={{bottom:`${targetHeight}%`}}/></div><span className="course-bar-label" title={row.course}>{compactCourseName(row.course)}</span><small>{number.format(row.vacancies)} vagas</small></div>})}

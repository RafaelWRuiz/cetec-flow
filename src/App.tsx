...
export default function App(){
 const [filters,setFilters]=useState<Filters>(initialFilters); const [selectedEtec,setSelectedEtec]=useState(''); const [geographicScope,setGeographicScope]=useState<{label:string;etecs:string[]}|null>(null); const [mapResetKey,setMapResetKey]=useState(0); const [analysisTab,setAnalysisTab]=useState<'evolution'|'regional'>('evolution'); const [statusFilter,setStatusFilter]=useState<OfferStatus|null>(null); const [dashboard,setDashboard]=useState<DashboardPayload|null>(null); const [snapshotStartAt,setSnapshotStartAt]=useState(''); const [snapshotEndAt,setSnapshotEndAt]=useState(''); const [importOpen,setImportOpen]=useState(false); const [importPassword,setImportPassword]=useState(''); const [importFile,setImportFile]=useState<File|null>(null); const [importMessage,setImportMessage]=useState(''); const [importing,setImporting]=useState(false)
 ...
 const {etecs,snapshotSeries,sourceMetadata}=dashboard??{etecs:staticEtecs,snapshotSeries:staticSnapshotSeries,sourceMetadata:staticSourceMetadata}; const rangeStartAt=snapshotStartAt||snapshotSeries.at(0)?.referenceAt||''; const rangeEndAt=snapshotEndAt||snapshotSeries.at(-1)?.referenceAt||''; const visibleSnapshotSeries=snapshotSeries.filter(snapshot=>snapshot.referenceAt>=rangeStartAt&&snapshot.referenceAt<=rangeEndAt); const selectedSnapshot=visibleSnapshotSeries.at(-1)??snapshotSeries.at(-1); const enrollments=selectedSnapshot?.enrollments??staticEnrollments
 const regionalByEtec=new Map(etecs.map(etec=>{const municipality=normalizeName(etec.municipality);return [etec.name,mapRegionalByMunicipality.get(municipality)??mapRegionalFallbackByMunicipality.get(municipality)]})); const regionalFor=(item:Enrollment)=>regionalByEtec.get(item.etec)??item.regional; const dependentKeys:FilterKey[]=['regional','etec','course','period']; const matchesFilters=(item:Enrollment,current:Filters,ignored?:FilterKey)=>dependentKeys.every(candidate=>candidate===ignored||!current[candidate]||(candidate==='regional'?regionalFor(item)===current.regional:item[candidate]===current[candidate])); const valuesFor=(key:FilterKey,current:Filters,scope=geographicScope)=>{const items=enrollments.filter(item=>(!scope||scope.etecs.includes(item.etec))&&matchesFilters(item,current,key)); const validItems=(key==='course'||key==='period')?items.filter(item=>!item.isTrainee):items; return [...new Set(validItems.map(item=>key==='regional'?regionalFor(item):item[key]))].sort()}; const mapRegionalOptions=REGIONAIS.features.map(feature=>String(feature.properties.regional??'')).filter(regional=>enrollments.some(item=>regionalFor(item)===regional)); const normalizeFilters=(next:Filters,preserve?:FilterKey,scope=geographicScope)=>{let normalized=next; let changed=true; while(changed){changed=false; dependentKeys.forEach(key=>{const values=key==='regional'?mapRegionalOptions:valuesFor(key,normalized,scope);if(key!==preserve&&normalized[key]&&!values.includes(normalized[key])){normalized={...normalized,[key]:''};changed=true}})} return normalized}; const options={regional:mapRegionalOptions,etec:valuesFor('etec',filters),modality:staticOptions.modality,course:valuesFor('course',filters),period:valuesFor('period',filters)}
 ...
 const matchesActiveScope=(item:Enrollment)=>matchesFilters(item,filters)&&(!geographicScope||geographicScope.etecs.includes(item.etec))&&(!statusFilter||(!item.isTrainee&&offerStatusFor(item)===statusFilter)); const scopedHistory=visibleSnapshotSeries.map(({referenceAt,enrollments:items})=>{const scoped=items.filter(matchesActiveScope);const paid=scoped.reduce((sum,item)=>sum+item.paid,0);const unpaid=scoped.reduce((sum,item)=>sum+item.unpaid,0);const trainee=scoped.filter(item=>item.isTrainee).reduce((sum,item)=>sum+item.paid+item.unpaid,0);const regularPaid=scoped.filter(item=>!item.isTrainee).reduce((sum,item)=>sum+item.paid,0);const vacancies=scoped.reduce((sum,item)=>sum+(item.isTrainee?0:item.vacancies),0);return {referenceAt,total:paid+unpaid,paid:regularPaid,unpaid,trainee,regular:paid+unpaid-trainee,vacancies}}); const data=Object.assign(enrollments.filter(matchesActiveScope),{history:scopedHistory}) as ChartData; const totals=data.reduce((r,item)=>({paid:r.paid+item.paid,unpaid:r.unpaid+item.unpaid,vacancies:r.vacancies+(item.isTrainee?0:item.vacancies),target:r.target+item.target,trainee:r.trainee+(item.isTrainee?item.paid+item.unpaid:0),regular:r.regular+(item.isTrainee?0:item.paid)}),{paid:0,unpaid:0,vacancies:0,target:0,trainee:0,regular:0}); const total=totals.paid+totals.unpaid
...
<select value={rangeStartAt} onChange={event=>{const next=event.target.value;setSnapshotStartAt(next);if(next>rangeEndAt)setSnapshotEndAt(next)}} aria-label="Data inicial">
...
<b>até</b>
<select value={rangeEndAt} onChange={event=>{const next=event.target.value;setSnapshotEndAt(next);if(next<rangeStartAt)setSnapshotStartAt(next)}} aria-label="Data final">
{snapshotSeries.map(snapshot=><option key={snapshot.referenceAt} value={snapshot.referenceAt}>{formatSnapshotDate(snapshot.referenceAt)}</option>)}
</select>
</div>
</label>
<span className="snapshot-status">{visibleSnapshotSeries.length} recortes no período<br/>
...
<D3GeographicMap etecs={etecs} selected={selectedEtec} visible={visible} focusedRegional={filters.regional} resetKey={mapResetKey} onSelect={selectEtec} onGeographicSelect={selectGeographicScope} onGeographicClear={clearGeographicScope}/>

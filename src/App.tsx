...
function CourseChart({rows,period,onPeriodChange}:{rows:CourseRow[];period:CoursePeriod;onPeriodChange:(period:CoursePeriod)=>void}){
 const plotRef=useRef<HTMLDivElement>(null); const [scaleMax,setScaleMax]=useState(5); const [trackMetrics,setTrackMetrics]=useState({height:280,bottom:52})
 useEffect(()=>{const plot=plotRef.current;if(!plot)return undefined;const updateScale=()=>{const bounds=plot.getBoundingClientRect();const visible=[...plot.querySelectorAll<HTMLElement>('[data-course-demand]')].filter(item=>{const rect=item.getBoundingClientRect();return rect.right>bounds.left&&rect.left<bounds.right}).map(item=>Number(item.dataset.courseDemand??0));const next=Math.max(5,...visible);setScaleMax(current=>Math.abs(current-next)<.01?current:next);const track=plot.querySelector<HTMLElement>('.course-bar-track');if(track){const rect=track.getBoundingClientRect();const metrics={height:rect.height,bottom:Math.max(0,bounds.bottom-rect.bottom)};setTrackMetrics(current=>Math.abs(current.height-metrics.height)<1&&Math.abs(current.bottom-metrics.bottom)<1?current:metrics)}};const observer=new ResizeObserver(updateScale);observer.observe(plot);plot.addEventListener('scroll',updateScale,{passive:true});updateScale();return()=>{observer.disconnect();plot.removeEventListener('scroll',updateScale)}},[rows])
 const targetHeight=Math.min(1.5/scaleMax*100,100)
...
<div className="course-plot-shell"><div className="course-y-axis" aria-hidden="true"><span className="course-y-max">{scaleMax.toFixed(1).replace('.',',')}x</span><span className="course-y-target" style={{bottom:`${targetHeight}%`}}>1,5x</span><span className="course-y-zero">0</span></div><div className="course-plot" ref={plotRef} role="img" aria-label="Demanda de inscritos pagos, não pagos e meta por curso"><i className="course-axis-line" aria-hidden="true" style={{bottom:`${trackMetrics.bottom}px`}}/><i className="course-target-line" aria-hidden="true" style={{bottom:`${trackMetrics.bottom+trackMetrics.height*targetHeight/100}px`}}/>
{rows.map(row=>{const totalDemand=row.vacancies?(row.paid+row.unpaid)/row.vacancies:0;const paidHeight=Math.min(row.vacancies?row.paid/row.vacancies/scaleMax*100:0,100);const unpaidHeight=Math.min(row.vacancies?row.unpaid/row.vacancies/scaleMax*100:0,100);return <div className="course-bar-group" data-course-demand={totalDemand} key={row.course}><div className="course-bar-values"><b>{number.format(row.paid)}</b><span>{number.format(row.unpaid)} não pagos</span></div><div className="course-bar-track"><i className="course-bar-paid" style={{height:`${paidHeight}%`}}/><i className="course-bar-unpaid" style={{bottom:`${paidHeight}%`,height:`${unpaidHeight}%`}}/></div><span className="course-bar-label" title={row.course}>{compactCourseName(row.course)}</span><small>{number.format(row.vacancies)} vagas</small></div>})}
